...
static void chargen_udp_classify(QNSM_PACKET_INFO *pkt_info, void *sess, void **arg)
{
    ...
    */
    ...
    if ((0 < len) && ((payload[0] < 0x20) || (0x7d < payload[0]))) {
        return;
    }

    pkt_info->dpi_app_prot = EN_QNSM_DPI_CHARGEN;

    QNSM_DEBUG(QNSM_DBG_M_DPI, QNSM_DBG_INFO, "leave\n");
    return;
}

int32_t chargen_reg(void)
{
    if (0 == qnsm_dpi_proto_enable(EN_QNSM_DPI_CHARGEN)) {
        return 0;
    }

    {
        /*reg classfy to dpi by l4proto+port*/
        qnsm_dpi_service_classify_reg(EN_DPI_PROT_UDP, CHARGEN_PORT, EN_QNSM_DPI_CHARGEN, chargen_udp_classify);
    }
    return 0;
}
